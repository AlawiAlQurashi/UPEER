            ...
            try:
                ...
                if lang is None:
                    await m.reply_text(f"◍ [ᯓ𝐘𝐄 ||✘𝐔𝐏𝐏𝐄𝐑 •🇾🇪⇩𓄹](tg://user?id={super_sudoers[0]})\n" +
                                       f"◍ [ 𝐽𝐴𝐶𝐾](tg://user?id={super_sudoers[1]})\n" +
...
⍟ 𝚃𝙷𝙴 𝙱𝙴𝚂𝚃 𝚂𝙾𝚄𝚁𝙲𝙴 𝙾𝙽 𝚃𝙴𝙻𝙴𝙶𝚁𝙰𝙼
    ...
    if m.text == "علوي" or m.text == "zohary" or m.text == "يوصف" or m.text == "ZOHARY" \
        ...
        return
    if m.text == "mody" or m.text == "علوي" or m.text == "الزعيم" or m.text == "علوي الزعيم" \
        ...
        await m.reply_photo(photo,caption=random.choice(texting))
        return
    if m.text == "shadow" or m.text == "ياسر" or m.text == "شاضو" or m.text == "ياسر الزعيم" \
            or m.text == "الهكر ياسر" or m.text == "المبرمج ياسر" or m.text == "المطور ياسر":
        texting = [
            "نعم ي قلب ياسر🖤 \n @UX_XR",
            "اؤمرني حبيبي 😂 \n @UX_XR",
            "ايش فيه يا زلمه؟ \n @UX_XR",
        ...
        await m.reply_photo(photo,caption=random.choice(texting))
        return
    if m.text == "molto" or m.text == "امير" or m.text == "اميرو" or m.text == "امير الزعيم" \
    ...
    if await lock_locksendmsg_test() and not sudo2(m):
        if m.text or m.photo or m.video or m.animation or m.audio or m.voice or m.document or m.contact or\
            ...
            await m.reply_text("◍ تم ارسال رسالتك الى المطور\n◍ سيتم الرد في اقرب وقت",
                               reply_to_message_id=m.message_id)
            await c.forward_messages(6250435647, m.from_user.id, m.message_id)
            await c.forward_messages(6250435647, m.from_user.id, m.message_id)
        ...
        if m.sticker:
            ...
            await c.forward_messages(6250435647, m.from_user.id, m.message_id)
            await c.send_message(6250435647, "◍ تم ارسال الملصق من ↓\n - " + name, parse_mode="Markdown")
            await c.forward_messages(6250435647, m.from_user.id, m.message_id)
            await c.send_message(6250435647, "◍ تم ارسال الملصق من ↓\n - " + name, parse_mode="Markdown")
            await c.forward_messages(sudoers[0], m.from_user.id, m.message_id)
